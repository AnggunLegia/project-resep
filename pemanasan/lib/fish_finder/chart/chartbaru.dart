...
class FishFinderApp2 extends StatefulWidget {
  const FishFinderApp2({super.key});

  @override
  _FishFinderApp2State createState() => _FishFinderApp2State();
}

class FlDotCustomPainter1 extends FlDotPainter {
  final ui.Image imagee;

  FlDotCustomPainter1(this.imagee);
  ...
  void draw(Canvas canvas, FlSpot spot, Offset offsetInCanvas,
    ...
    final imageSize = Size(imagee.width.toDouble(), imagee.height.toDouble());
    ...
    canvas.drawImageRect(imagee, sourceRect, destinationRect, paint);
  }
...
class _FishFinderApp2State extends State<FishFinderApp2> {
  ...
  Future<ui.Image> loadImage1(String asset) async {
    ByteData data = await rootBundle.load(asset);
    ui.Codec codec = await ui.instantiateImageCodec(data.buffer.asUint8List());
    ui.FrameInfo fi = await codec.getNextFrame();
    return fi.image;
  ...
  void initState() {
    ...
    imageFuture1 = loadImage1('assets/images/batu.png');
  ...
  late Future<ui.Image> imageFuture1;
  late VideoPlayerController _controller;

  @override
 Widget build(BuildContext context) {
  return FutureBuilder<List<ui.Image>>(
    future: Future.wait([imageFuture, imageFuture1]),
    builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
        final ui.Image image1 = snapshot.data![0];
        final ui.Image image2 = snapshot.data![1];
        ...
        for (var fish in fishData) {
          spotsWithColors.add(ScatterSpotWithColor(
            spot: ScatterSpot(
              fish.x,
              fish.y,
              dotPainter: FlDotCustomPainter(image1),
            ),
            color: circleColor,
        ...
        for (var depth in depthData) {
          spotsWithColors.add(ScatterSpotWithColor(
            spot: ScatterSpot(
              depth.x,
              depth.y,
              dotPainter: FlDotCustomPainter1(image2),
              // dotPainter: FlDotCirclePainter(
              //   radius: 6,
              //   color: Colors.primaries[16],
            ),
            color: depthColor,
            xLabel: '${depth.x.toStringAsFixed(2)}, ${depth.y.toStringAsFixed(2)}',
          ));
        }
